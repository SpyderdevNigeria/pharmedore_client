import s from './RetailDrugCards.module.css'
...
export default function RetailDrugCards({ drugs, loading }) {
        ...
        <div className={s.cards}>
          {filteredDrugs.map((drug, i) => (
            <div className={s.card} key={i}>
              <Link to={`/dashboard/retailer/retail-drug-detail/${drug._id}`} className={s.img}>
                <DrugListIcon className={s.drugIcon}/>
              </Link>

              <div className={s.info}>
                <Link to={`/dashboard/retailer/retail-drug-detail/${drug._id}`} className={s.name}>
                  {drug.drugName}<br/>
                  <span>({drug.genericName})</span>
                </Link>

                <p className={s.price}>₦‎{drug.forms[0]?.price}</p>
                <Link to={`/dashboard/retailer/retail-drug-detail/${drug._id}`} className={s.infoCtn}>
                  <FaInfo />
                </Link>
              </div>
            </div>
