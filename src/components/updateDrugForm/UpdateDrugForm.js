...
export default function UpdateDrugForm({ type, Drug }) {
  ...
  useEffect(() => {
    setDrugCompany(Drug.drugCompany)
    setDrugName(Drug.drugName)
    setDrugGenericName(Drug.genericName)
    setDrugCategory(Drug.category)
    setForms(Drug.forms)
    setStrengths(Drug.strengths)
    setImages(Drug.images)
  ...
  return (
    <div className={`formCtn ${s.ctn}`}>
      <form onSubmit={handleSubmit} className={`formWrp adminWrp ${s.wrp}`} id='UpdateDrugForm'>
        ...
        <button className='bigBtn formBtn' onClick={handleSubmit}>
          {loading? <ImSpinner8 className="spin" /> : 'Update Drug'}
        </button>

        {error && <p className='formError'>{error}</p>}
        {success && <p className='formSuccess'>Drug Updated Successfully</p>}
      </form>
    </div>
  )
}